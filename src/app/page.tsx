      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-border pb-6 mb-8 gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-slate-950/80 px-4 py-3 rounded-lg border border-border">
          ...
          <button 
            onClick={syncWithDb} 
            disabled={isSyncing}
            className={`flex items-center gap-2 font-mono text-xs font-semibold px-2.5 py-1 rounded transition-colors ${
              isSyncing 
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="glass-card rounded-xl p-6 neon-glow-border transition-all">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-xs font-mono text-slate-400">INTERACTIVE WEEK GRID</span>
            </div>
                    ...
                    {currentWeekDates.map((day, idx) => {
                      ...
                      return (
                        <th key={idx} className="text-center pb-3 px-1">
                          <div className={`flex flex-col items-center justify-center p-1.5 rounded min-w-[44px] ${
                            isToday ? 'bg-cyber-neon/10 border border-cyber-neon/30' : ''
                          }`}>
                            <span className={`text-xs font-mono font-bold uppercase ${
                              isToday ? 'text-cyber-neon' : 'text-slate-400'
                            }`}>
                              {DAYS_OF_WEEK[day.dayOfWeek].label}
                            </span>
                            <span className={`text-[10px] font-mono mt-0.5 ${
                              isToday ? 'text-cyber-neon/80' : 'text-slate-600'
                            }`}>
                              {day.dayOfMonth}
                            </span>
                          </div>
          ...
          <div className="glass-card rounded-xl p-6 border border-slate-800">
            ...
            <form onSubmit={handleAddHabitSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-mono text-slate-400 uppercase tracking-widest mb-1.5">Frequency Protocol</label>
                  <select
                    value={newHabitFreq}
              ...
              >
                <Plus className="h-4 w-4" /> Install Habit Module
              </button>
            </form>
          </div>
        </div>
